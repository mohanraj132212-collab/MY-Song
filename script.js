...
function loadStats() {
  try {
    ...
    if (s) { const d = JSON.parse(s); playCounts = d.playCounts || {}; listenSeconds = d.listenSeconds || {}; }
    const h = localStorage.getItem(HISTORY_KEY);
    if (h) { playHistory = JSON.parse(h) || []; }
  } catch (e) {}
}
function saveStats() {
  try {
    localStorage.setItem(STATS_KEY, JSON.stringify({ playCounts, listenSeconds }));
    localStorage.setItem(HISTORY_KEY, JSON.stringify(playHistory.slice(-500)));
  } catch (e) {}
}
function recordPlay(trackId) {
  playCounts[trackId] = (playCounts[trackId] || 0) + 1;
  sessionStarted[trackId] = Date.now();
  playHistory.push({ trackId, timestamp: Date.now(), duration: 0 });
  saveStats();
  updateTotalPlaysUI();
}
function recordListenTime(trackId, seconds) {
  if (seconds < 2) return;
  listenSeconds[trackId] = (listenSeconds[trackId] || 0) + seconds;
  if (playHistory.length > 0) {
    const last = playHistory[playHistory.length - 1];
    if (last.trackId === trackId) last.duration = seconds;
  }
  saveStats();
}
function updateTotalPlaysUI() {
...
(async () => {
  ...
  if (saved) {
    try {
      const snap = await get(userRef(saved));
      if (snap.exists()) { await loginWithData(saved, snap.val()); return; }
    } catch (e) {}
    clearSession();
  }
})();
...
window.togglePassVis = (inputId, iconEl) => {
  ...
  if (inp.type === 'password') {
    inp.type = 'text';
    iconEl.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>`;
  } else {
    inp.type = 'password';
    iconEl.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>`;
  }
};
...
window.checkPwStrength = (input) => {
  ...
  fill.style.background = ['#ff4444', '#ff8800', '#ffcc00', '#99dd00', '#1db954'][Math.min(s - 1, 4)] || 'transparent';
};
...
window.doSignup = async () => {
  ...
  try {
    ...
    if (snap.exists()) { showFieldErr('signupUsernameErr', 'Username already taken'); setAuthLoading('signup', false); return; }
    ...
    await set(userRef(username), userData);
    saveSession(username);
    await loginWithData(username, userData);
...
window.doLogin = async () => {
  ...
  try {
    const snap = await get(userRef(username));
    if (!snap.exists()) { showFieldErr('loginUsernameErr', 'Username not found'); setAuthLoading('login', false); return; }
    const data = snap.val();
    const passwordHash = await hashPassword(pass);
    if (data.passwordHash !== passwordHash) { showFieldErr('loginPassErr', 'Incorrect password'); setAuthLoading('login', false); return; }
    saveSession(username);
    await loginWithData(username, data);
...
function applyUserUI() {
  ...
  document.getElementById('topbarAvatar').src = src;
  ...
  document.getElementById('settingsPhone').textContent = phone || 'Not set';
...
window.doLogout = () => {
  ...
  document.getElementById('avatarPlaceholderSvg').style.display = 'block';
  switchAuthTab('login');
};
...
function initApp() {
  renderLibContent();
  filterTracks('all', null);
  renderQueueState();
  document.getElementById('plPlayBtn').onclick = () => { if (currentPlaylistId) playPlaylist(currentPlaylistId); };
  document.getElementById('plAddSongsBtn').onclick = () => { if (currentPlaylistId) openAddSongsModal(currentPlaylistId); };
  document.getElementById('plDeleteBtn').onclick = () => { if (currentPlaylistId) confirmDeletePlaylist(currentPlaylistId); };

  // ── Wire up the seek bar range input ──
  // The progress bar visual click (legacy) is replaced by the overlay <input range>
  // npProgInput is declared in index.html oninput="seekFromInput(this.value)"
  // but we also attach it here for safety
  const progInput = document.getElementById('npProgInput');
  if (progInput) {
    progInput.addEventListener('input', () => seekFromInput(progInput.value));
  }

  // ── Wire up the volume range input ──
  const volInput = document.getElementById('npVolInput');
  if (volInput) {
    volInput.value = volume * 100;
    syncVolSliderTrack(volume * 100);
    volInput.addEventListener('input', () => setVolumeFromInput(volInput.value));
  }
}
...
// ══════════════════════════════════════════════════════
// NOW PLAYING UI — updateAllUI
// Syncs every visual element of the NP overlay + mini player
// ══════════════════════════════════════════════════════
function updateAllUI() {
  ...
  if (t.cover) {
    document.getElementById('miniArtImg').src = t.cover;
    document.getElementById('miniArtImg').style.display = 'block';
    document.getElementById('miniArtPlaceholder').style.display = 'none';
  } else {
    document.getElementById('miniArtImg').style.display = 'none';
    document.getElementById('miniArtPlaceholder').style.display = 'block';
  }
  ...
  if (t.cover) {
    document.getElementById('npCardArtImg').src = t.cover;
    document.getElementById('npCardArtImg').style.display = 'block';
    document.getElementById('npCardArtPlaceholder').style.display = 'none';
  } else {
    document.getElementById('npCardArtImg').style.display = 'none';
    document.getElementById('npCardArtPlaceholder').style.display = 'block';
  }
  ...
  if (t.cover) {
    document.getElementById('npArtImg').src = t.cover;
    document.getElementById('npArtImg').style.display = 'block';
    document.getElementById('npArtPlaceholder').style.display = 'none';
    document.getElementById('npBg').style.backgroundImage = `url(${t.cover})`;
  } else {
    document.getElementById('npArtImg').style.display = 'none';
    document.getElementById('npArtPlaceholder').style.display = 'flex';
    document.getElementById('npArtPlaceholder').textContent = ['♪', '♫', '♬', '♩'][t.id % 4];
...
function renderQueuePanel() {
  ...
  if (queue.length === 0) {
    us.innerHTML = `<div class="queue-up-next"><div class="queue-up-label">Up Next</div></div><div class="queue-empty"><div class="queue-empty-icon">🎵</div><div>Queue is empty</div><div style="font-size:11px;margin-top:4px;color:var(--txt3)">Use ⋮ on any song to add</div></div>`;
  } else {
    ...
    queue.forEach((trackIdx, i) => {
      ...
      if (!t) return;
      const row = document.createElement('div');
      row.className = 'queue-track-item';
...
function updateLikeBtns() {
  ...
  document.getElementById('npCardLikeBtn').innerHTML = heart(16);
  document.getElementById('npCardLikeBtn').classList.toggle('liked', liked);
  document.getElementById('npLikeBtn').innerHTML = heart(24);
  document.getElementById('npLikeBtn').classList.toggle('liked', liked);
}
...
window.togglePlay = () => {
  ...
  if (isPlaying) {
    ...
    if (currentTrackIdx >= 0 && sessionStarted[tracks[currentTrackIdx].id]) {
      const el = (Date.now() - sessionStarted[tracks[currentTrackIdx].id]) / 1000;
      recordListenTime(tracks[currentTrackIdx].id, el);
      delete sessionStarted[tracks[currentTrackIdx].id];
    }
  } else {
    audio.play();
    isPlaying = true;
...
window.toggleLike = () => {
  ...
  updateLikeBtns();
  saveUserData();
  if (currentFilter === 'liked') filterTracks('liked', null);
};

// ══════════════════════════════════════════════════════
// SEEK BAR
// The visual bar (npProgFill width) is updated by timeupdate.
// The invisible <input range id="npProgInput"> lets the user scrub.
// npProgBar click is kept as a fallback for browsers that
// might not receive the range input events.
// ══════════════════════════════════════════════════════

// Called from oninput on #npProgInput in index.html
window.seekFromInput = (value) => {
  if (!audio.duration) return;
  audio.currentTime = (value / 100) * audio.duration;
  // Keep fill in sync immediately (timeupdate will also update it)
  document.getElementById('npProgFill').style.width = value + '%';
};

// Fallback click handler on the visual bar itself
document.getElementById('npProgBar').addEventListener('click', e => {
  // Don't double-fire if the range input absorbed the event
...
// ══════════════════════════════════════════════════════
// VOLUME
// Uses <input type="range" id="npVolInput"> (class np-vol-input)
// CSS gradient trick: --vol-pct CSS variable fills the track
// ══════════════════════════════════════════════════════

// Sync the CSS gradient fill on the range track
function syncVolSliderTrack(value) {
  const el = document.getElementById('npVolInput');
  if (el) el.style.setProperty('--vol-pct', value + '%');
}

// Called from oninput on #npVolInput in index.html
window.setVolumeFromInput = (value) => {
  volume = value / 100;
  if (!isMuted) audio.volume = volume;
  syncVolSliderTrack(value);
};

// ── Mute toggle — updates the mute button icon + audio volume ──
window.toggleMute = () => {
  isMuted = !isMuted;
  audio.volume = isMuted ? 0 : volume;

  const muteBtn = document.getElementById('npMuteBtn');
  if (!muteBtn) return;

  if (isMuted) {
    // Crossed-out speaker (muted)
    muteBtn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
audio.addEventListener('timeupdate', () => {
  ...
  // NP overlay visual fill
  document.getElementById('npProgFill').style.width = pct + '%';

  // Keep the range input thumb in sync so scrubbing starts from current position
  const progInput = document.getElementById('npProgInput');
  if (progInput && !progInput.matches(':active')) {
    progInput.value = pct;
  }
...
audio.addEventListener('loadedmetadata', () => {
  ...
  if (currentTrackIdx >= 0) durations[tracks[currentTrackIdx].id] = audio.duration;
  // Reset seek input range to 0 on new track
  const progInput = document.getElementById('npProgInput');
...
window.setSleepTimer = (mins, el) => {
  ...
  sleepTimer = setTimeout(() => {
    audio.pause(); isPlaying = false; updatePlayIcons(false);
    showToast('Sleep timer ended — music stopped');
    document.getElementById('sleepIndicator').classList.remove('active');
    document.getElementById('sleepFieldValue').textContent = 'Off';
    clearInterval(sleepInterval); sleepTimer = null;
  }, mins * 60000);
  document.getElementById('sleepIndicator').classList.add('active');
  document.getElementById('sleepLabel').textContent = `${mins}m`;
  document.getElementById('sleepFieldValue').textContent = `${mins} min`;
  clearInterval(sleepInterval);
  sleepInterval = setInterval(() => {
    const rem = Math.max(0, Math.round((sleepEnd - Date.now()) / 60000));
    document.getElementById('sleepLabel').textContent = `${rem}m`;
    document.getElementById('sleepFieldValue').textContent = rem > 0 ? `${rem} min` : 'Off';
    if (rem <= 0) clearInterval(sleepInterval);
  }, 30000);
  showToast(`Sleep timer: ${mins} minutes`);
};
window.cancelSleepTimer = () => {
  clearTimeout(sleepTimer); clearInterval(sleepInterval); sleepTimer = null;
  document.getElementById('sleepIndicator').classList.remove('active');
  document.getElementById('sleepFieldValue').textContent = 'Off';
  document.querySelectorAll('.sleep-opt').forEach(o => o.classList.remove('active'));
...
function renderLibContent() {
  ...
  if (currentLibTab === 'playlists') {
    ...
    lc.className = 'pl-card';
    lc.innerHTML = `<div class="pl-card-art liked-card-art"><svg width="22" height="22" viewBox="0 0 24 24" fill="var(--accent)"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></div><div class="pl-card-info"><div class="pl-card-name">Liked Songs</div><div class="pl-card-sub">${likedSet.size} songs</div></div>`;
    lc.onclick = () => { document.querySelectorAll('.lib-tab').forEach(t => t.classList.remove('active')); document.querySelectorAll('.lib-tab')[1].classList.add('active'); switchLibTab('liked', null); };
    c.appendChild(lc);
    ...
    if (!liked.length) { c.innerHTML = '<div style="padding:40px;text-align:center;color:var(--txt3);font-size:14px">No liked songs yet</div>'; return; }
...
window.openPlDetail = (id) => {
  ...
  const c = document.getElementById('plDetailTrackList');
  c.innerHTML = '';
  songs.forEach((t, i) => renderTrackRow(t, i, tracks.indexOf(t), c));
...
window.playPlaylist = (id) => {
  const pl = playlists[id];
  if (!pl || !pl.songIds?.length) { showToast('No songs in playlist'); return; }
  const first = tracks.findIndex(t => t.id === pl.songIds[0]);
  if (first >= 0) playTrack(first);
  queue = pl.songIds.slice(1).map(sid => tracks.findIndex(t => t.id === sid)).filter(i => i >= 0);
  renderQueueState();
  closePlDetail();
};
window.confirmDeletePlaylist = (id) => {
  const pl = playlists[id];
  if (!pl) return;
  document.getElementById('deleteConfirmSub').textContent = `Delete "${pl.name}"? This cannot be undone.`;
  document.getElementById('deleteConfirmOk').onclick = () => { delete playlists[id]; saveUserData(); closeDeleteConfirm(); closePlDetail(); showToast('Playlist deleted'); };
...
window.createPlaylist = () => {
  ...
  playlists[id] = { name, cover: plArtDataUrl, songIds: [] };
  saveUserData();
  renderLibContent();
...
// ── SETTINGS ──
window.updateProfileAvatar = async (input) => {
  ...
  r.onload = async e => {
    ...
    if (currentUser) {
      ...
      try { await update(userRef(currentUser.username), { avatarUrl: url }); showToast('Profile picture updated'); }
...
window.saveProfile = async () => {
  ...
  try {
    await update(userRef(currentUser.username), { displayName: name, phone: phone || '' });
...
function getStatsForPeriod(period) {
  if (period === 'all') {
    ...
    const totalSeconds = Object.values(listenSeconds).reduce((a, b) => a + b, 0);
    const songsPlayed = Object.keys(playCounts).filter(id => playCounts[id] > 0).length;
    const topTracks = Object.entries(playCounts).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([id, count]) => ({ track: tracks.find(t => t.id == id), count, secs: listenSeconds[id] || 0 })).filter(x => x.track);
    return { totalPlays, totalSeconds, songsPlayed, topTracks };
  } else {
    const hist = getFilteredHistory(period);
    const pc = {}, ps = {};
    hist.forEach(h => { pc[h.trackId] = (pc[h.trackId] || 0) + 1; ps[h.trackId] = (ps[h.trackId] || 0) + (h.duration || 0); });
    const totalPlays = hist.length, totalSeconds = Object.values(ps).reduce((a, b) => a + b, 0), songsPlayed = Object.keys(pc).length;
    const topTracks = Object.entries(pc).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([id, count]) => ({ track: tracks.find(t => t.id == id), count, secs: ps[id] || 0 })).filter(x => x.track);
    return { totalPlays, totalSeconds, songsPlayed, topTracks };
  }
}
...
function renderStatsPage() {
  ...
  const totalAllTimePlays = Object.values(playCounts).reduce((a, b) => a + b, 0);
  const periodTabsHtml = `<div style="padding:16px 16px 0;"><div class="stats-period-tabs"><button class="stats-period-tab ${statsPeriod === 'week' ? 'active' : ''}" onclick="setStatsPeriod('week')">1 Week</button><button class="stats-period-tab ${statsPeriod === 'month' ? 'active' : ''}" onclick="setStatsPeriod('month')">1 Month</button><button class="stats-period-tab ${statsPeriod === '3months' ? 'active' : ''}" onclick="setStatsPeriod('3months')">3 Months</button><button class="stats-period-tab ${statsPeriod === 'all' ? 'active' : ''}" onclick="setStatsPeriod('all')">All Time</button></div></div>`;
  const heroHtml = `<div style="padding:0 16px 16px;"><div class="stats-hero"><div class="stats-hero-row"><div class="stats-hero-main"><div class="stats-big-num">${stats.totalPlays}</div><div class="stats-big-label">Total Plays</div></div><div class="stats-hero-side"><div class="stats-side-item"><div class="stats-side-num">${fmtTimeVerbose(stats.totalSeconds)}</div><div class="stats-side-label">Listening Time</div></div><div class="stats-side-item"><div class="stats-side-num">${stats.songsPlayed}</div><div class="stats-side-label">Songs Played</div></div></div></div></div></div>`;
  const avgPerDay = statsPeriod === 'week' ? (stats.totalPlays / 7).toFixed(1) : statsPeriod === 'month' ? (stats.totalPlays / 30).toFixed(1) : statsPeriod === '3months' ? (stats.totalPlays / 90).toFixed(1) : (totalAllTimePlays / Math.max(1, Math.ceil((Date.now() - (playHistory[0]?.timestamp || Date.now())) / (86400000)))).toFixed(1);
  const miniCardsHtml = `<div class="stats-cards-row"><div class="stats-mini-card"><div class="stats-mini-icon" style="background:rgba(29,185,84,.12);"><svg width="18" height="18" viewBox="0 0 24 24" fill="var(--txt3)"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg></div><div class="stats-mini-num" style="color:var(--accent)">${tracks.length}</div><div class="stats-mini-label">Songs in Library</div></div><div class="stats-mini-card"><div class="stats-mini-icon" style="background:rgba(74,158,255,.12);"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/></svg></div><div class="stats-mini-num" style="color:var(--stats-blue)">${avgPerDay}</div><div class="stats-mini-label">Avg plays/day</div></div><div class="stats-mini-card"><div class="stats-mini-icon" style="background:rgba(244,185,66,.12);"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg></div><div class="stats-mini-num" style="color:var(--stats-gold)">${likedSet.size}</div><div class="stats-mini-label">Liked Songs</div></div><div class="stats-mini-card"><div class="stats-mini-icon" style="background:rgba(168,85,247,.12);"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg></div><div class="stats-mini-num" style="color:var(--stats-purple)">${Object.keys(playlists).length}</div><div class="stats-mini-label">Playlists</div></div></div>`;
  let topSongsHtml = '';
  if (stats.topTracks.length === 0) {
    topSongsHtml = `<div class="stats-section"><div class="stats-section-title"> Top Songs <span class="badge" style="background:var(--bg3);color:var(--txt3)">0</span></div><div class="stats-empty"><div class="stats-empty-icon">🎧</div><div style="font-size:15px;font-weight:700;color:var(--txt2)">No plays yet</div><div style="font-size:13px;margin-top:6px">Start playing music to see your stats!</div></div></div>`;
  } else {
    const maxCount = stats.topTracks[0]?.count || 1;
    const rankColors = ['top1', 'top2', 'top3'];
    const songRows = stats.topTracks.map((item, i) => {
      const pct = Math.round((item.count / maxCount) * 100);
      return `<div class="stats-song-item"><div class="stats-song-rank ${rankColors[i] || ''}">${i + 1}</div><div class="stats-song-art">${thumbHtml(item.track)}</div><div class="stats-song-info"><div class="stats-song-name">${item.track.name}</div><div class="stats-song-artist">${item.track.artist}</div><div class="stats-prog-wrap"><div class="stats-prog-fill" style="width:${pct}%"></div></div></div><div class="stats-song-right"><div class="stats-play-count">${item.count}</div><div class="stats-play-label">play${item.count !== 1 ? 's' : ''}</div>${item.secs > 0 ? `<div class="stats-time-label">${fmtTimeVerbose(item.secs)}</div>` : ''}</div></div>`;
    }).join('');
    topSongsHtml = `<div class="stats-section"><div class="stats-section-title"> Top Songs <span class="badge" style="background:var(--accent-dim);color:var(--accent)">${stats.topTracks.length}</span></div>${songRows}</div>`;
  }
  scroll.innerHTML = periodTabsHtml + heroHtml + miniCardsHtml + topSongsHtml;
}
...
document.addEventListener('keydown', e => {
  ...
  else if (e.code === 'Escape') { closeNowPlaying(); closeQueuePanel(); closeStatsPage(); closeCropModal(); }
});
...
tracks.forEach(t => {
  ...
  a.preload = 'metadata';
  a.src = t.url;
  a.addEventListener('loadedmetadata', () => { durations[t.id] = a.duration; });
});
