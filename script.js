    ...
    // The single function that dismisses the splash and reveals
    // the correct screen. Called exactly once per page load.
    // ══════════════════════════════════════════════════════════
    function resolveAuth(showScreen) {
      // showScreen is 'app' or 'auth'
      ...
      if (showScreen === 'app') {
        ...
        // Micro-delay so the browser renders the element before
        // triggering the CSS opacity transition for a smooth fade-in.
    ...
    // ══════════════════════════════════════════════════════════
    // BOOT — Session check on every page load.
    //
    // Timeline:
    //   t=0ms   : Splash visible (no auth screen visible at all)
    //   t=0ms   : Firebase SDK init begins
    //   t=Xms   : Firebase get() resolves (network round-trip)
    //   t=X+1ms : resolveAuth() called → correct screen fades in,
    //             splash fades out simultaneously.
    //
    // Returning users never see the auth screen. New/logged-out
    // users see the splash → auth screen (no app screen flash).
    // ══════════════════════════════════════════════════════════
    (async () => {
      ...
      if (saved) {
        try {
          ...
          if (snap.exists()) {
            await loginWithData(saved, snap.val(), /* skipResolve= */ false);
            return; // resolveAuth('app') called inside loginWithData
          }
        } catch (e) {
          // Network error — clear stale session, fall through to auth
        }
        clearSession();
      }
    ...
    // ══════════════════════════════════════════════════════════
    // loginWithData — sets up app state and triggers screen
    // reveal. Called from:
    //   1. Session restore (boot IIFE) — resolves splash → app
    //   2. Manual login / signup — resolves auth → app
    // ══════════════════════════════════════════════════════════
    async function loginWithData(username, data) {
      ...
      // Hide auth (if visible) and show app
      const authEl = document.getElementById('authScreen');
      authEl.classList.remove('active', 'visible');
    ...
    window.doLogout = () => {
      ...
      audio.pause(); isPlaying = false; currentUser = null; likedSet.clear(); playlists = {}; queue = [];
      clearSession();
      // Remove session class so next load shows auth immediately via splash
      document.documentElement.classList.remove('has-session');

      const appEl = document.getElementById('appScreen');
      const authEl = document.getElementById('authScreen');
      appEl.classList.remove('active', 'visible');
      authEl.classList.add('active');
    ...
    function renderTracks(list) { const c = document.getElementById('trackList'); if (!c) return; c.innerHTML = ''; list.forEach((t,i) => renderTrackRow(t,i,tracks.indexOf(t),c)); }
    ...
    window.setSleepTimer = (mins, el) => {
      document.querySelectorAll('.sleep-opt').forEach(o=>o.classList.remove('active')); el.classList.add('active');
      sleepEnd = Date.now()+mins*60000; clearTimeout(sleepTimer); clearInterval(sleepInterval);
    ...
    window.cancelSleepTimer = () => {
      ...
      closeModal('sleepModal'); showToast('Sleep timer cancelled');
    };
    ...
    window.openPlDetail = (id) => {
      currentPlaylistId=id; const pl=playlists[id]; if(!pl) return;
    ...
    window.playPlaylist = (id) => {
      const pl=playlists[id]; if(!pl||!pl.songIds?.length) { showToast('No songs in playlist'); return; }
      const first=tracks.findIndex(t=>t.id===pl.songIds[0]); if(first>=0) playTrack(first);
    ...
    window.createPlaylist = () => {
      const name=document.getElementById('newPlName').value.trim(); if(!name) { showToast('Enter a playlist name'); return; }
      const id='pl_'+Date.now(); playlists[id]={name,cover:plArtDataUrl,songIds:[]};
      saveUserData(); renderLibContent(); closeModal('newPlaylistModal'); showToast(`Playlist "${name}" created`); openPlDetail(id);
    };
    window.openAddSongsModal = (plId) => {
      addSongModalPlaylistId=plId; const pl=playlists[plId]; addSongModalSelected=new Set(pl?.songIds||[]);
      document.getElementById('addSongsSearch').value=''; renderAddSongsModal(tracks); openModal('addSongsModal');
    };
    ...
    window.saveProfile = async () => {
      const name=document.getElementById('editName').value.trim(); const phone=document.getElementById('editPhone').value.trim();
      if (!name) { showToast('Enter a display name'); return; }
      try { await update(userRef(currentUser.username),{displayName:name,phone:phone||''}); currentUser.displayName=name; currentUser.phone=phone; applyUserUI(); closeModal('editProfileModal'); showToast('Profile updated'); }
    ...
    function getStatsForPeriod(period) {
      if (period==='all') {
        const totalPlays=Object.values(playCounts).reduce((a,b)=>a+b,0), totalSeconds=Object.values(listenSeconds).reduce((a,b)=>a+b,0), songsPlayed=Object.keys(playCounts).filter(id=>playCounts[id]>0).length;
        ...
        const hist=getFilteredHistory(period), pc={}, ps={};
    ...
    function renderStatsPage() {
      ...
      if (stats.topTracks.length===0) {
        ...
        const maxCount=stats.topTracks[0]?.count||1; const rankColors=['top1','top2','top3'];
        const songRows=stats.topTracks.map((item,i)=>{ const pct=Math.round((item.count/maxCount)*100); return `<div class="stats-song-item"><div class="stats-song-rank ${rankColors[i]||''}">${i+1}</div><div class="stats-song-art">${thumbHtml(item.track)}</div><div class="stats-song-info"><div class="stats-song-name">${item.track.name}</div><div class="stats-song-artist">${item.track.artist}</div><div class="stats-prog-wrap"><div class="stats-prog-fill" style="width:${pct}%"></div></div></div><div class="stats-song-right"><div class="stats-play-count">${item.count}</div><div class="stats-play-label">play${item.count!==1?'s':''}</div>${item.secs>0?`<div class="stats-time-label">${fmtTimeVerbose(item.secs)}</div>`:''}</div></div>`; }).join('');
